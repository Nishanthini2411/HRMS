...
import { getManagerSession } from "./managerApproverData"; // உங்கள் session helper
...
export default function ManagerApprovals() {
  ...
  const canAct = (session.role || session.access) === "approver";
  ...
  const handleAction = async (rowId, nextStatus) => {
    ...
    const { error } = await supabase.from(LEAVES_TABLE).update(payload).eq("id", rowId);
    ...
    <div className="space-y-4">
      <div className="rounded-2xl border bg-indigo-50 p-5 flex flex-col gap-2">
        ...
        <p className="text-sm text-indigo-800/80">
          Two managers sign in with separate IDs. Only the approver can approve or reject; the second manager has read-only visibility.
        </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {loading ? (
          ...
          <div className="rounded-2xl border p-4 bg-white text-sm text-slate-600">No requests found for this manager.</div>
        ) : (
          requests.map((req) => {
